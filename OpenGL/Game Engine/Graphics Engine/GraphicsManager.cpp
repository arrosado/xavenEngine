...
void GraphicsManager::Init(int argc, ...) {
    ...
    this->iva = (TexturedColoredVertex*)calloc(kMax_Images, sizeof(TexturedColoredVertex));
	this->ivaIndices = (GLushort*)calloc(kMax_Images * 6, sizeof(GLushort));
...
void GraphicsManager::CopyImageDetails(ImageDetails* imageDetails) {
	...
	if (ivaIndex + 1 > kMax_Images) {
		printf("ERROR - GraphicsManager: Render queue size exceded. Consider increasing the default size.");
		throw "ERROR - GraphhicsManager";
	}
...
void GraphicsManager::HandleEvents() {
	...
	if (up || down || right || left)
		this->m_camera->Move(right && left ? 0.0f : right ? -SPEED : left ? SPEED : 0.0f,
							 0.0f, 
