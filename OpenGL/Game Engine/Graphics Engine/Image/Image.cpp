...
void Image::Render() {
    ...
    if (this->dirty) {
        ...
        if(this->flipVertically) {
            scaleMatrix(this->matrix, Vector2DfMake(1, -1));
            translateMatrix(this->matrix, Vector2DfMake(0, (-this->imageSize.height * this->scale.y)));
        }
		
        if(this->flipHorizontally) {
            scaleMatrix(this->matrix, Vector2DfMake(-1, 1));
            translateMatrix(this->matrix, Vector2DfMake((-this->imageSize.width * this->scale.x), 0));
        }
...
void Image::Render(Vector2Df point, Vector2Df scale, float rotation) {
	this->point = point;
	this->scale = scale;
...
void Image::Render(bool centered) {
	this->dirty = true;
	this->Render(Vector2DfMake(this->point.x, this->point.y), Vector2DfMake(1.0f, 1.0f), this->rotation, centered);
}
...
void Image::Render(Vector2Df point, Vector2Df scale, float rotation, bool centered) {
	
	this->scale = scale;
	this->rotation = rotation;
    if (centered) {
        this->rotationPoint.x = ((this->imageSize.width * scale.x) / 2);
        this->rotationPoint.y = ((this->imageSize.height * scale.y) / 2);
    }
	this->point.x = point.x - ((this->imageSize.width * scale.x) / 2);
	this->point.y = point.y - ((this->imageSize.height * scale.y) / 2);
	this->dirty = true;
	this->Render();
